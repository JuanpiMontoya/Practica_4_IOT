...
function calculateWaterLimit(area, plantType) {
    ...
    switch (plantType) {
        case 'cesped':
            ...
            break;
        case 'flores':
            waterPerSquareMeter = 12;  // 12 litros por metro cuadrado
            break;
        case 'cultivos':
            waterPerSquareMeter = 15; // 15 litros por metro cuadrado
            break;
        case 'arboles':
            waterPerSquareMeter = 20; // 20 litros por metro cuadrado
            break;
        case 'arbustos':
            waterPerSquareMeter = 12; // 12 litros por metro cuadrado
            break;
        case 'vegetales':
            waterPerSquareMeter = 14; // 14 litros por metro cuadrado
            break;
        default:
            waterPerSquareMeter = 10; // Valor por defecto
    }
...
const CapturePlantTypeIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        const params = {
            thingName: thingName,
            payload: JSON.stringify({ state: { desired: { water_limit: waterLimit}, reported: { area: area, plant_type: plantType} } })
        };
...
const CloseValveIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        const params = {
            thingName: thingName,
            payload: JSON.stringify({ state: { desired: { watering: 0 } } })
        };
...
const OpenValveIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        const params = {
            thingName: thingName,
            payload: JSON.stringify({ state: { desired: { watering: 1 } } })
        };
...
const HelpIntentHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Puedes decir "abrir la válvula", "cerrar la válvula", "consultar el estado" o "configurar dispositivo". ¿Cómo te puedo ayudar?';
